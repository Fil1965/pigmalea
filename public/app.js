...
async function enhanceImage() {
  ...
  try {
    ...
    // ---- Client-side super-resolution (UpscalerJS / ESRGAN 2x) with server fallback ----
    const wantsUpscale = document.getElementById('check-upscale').checked;
    if (wantsUpscale && data.image.ai_upscale_recommendation !== false) {
      // Loader stays visible during super-resolution (btn/loader are in function scope)
      btn.classList.add('hidden');
      loader.classList.remove('hidden');
      try {
        await applySuperResolution(data.image, wantsUpscale);
      } catch (e) {
        console.error('Super-resolution orchestration failed:', e);
        showToast(`Súper-resolución falló: ${e.message}`, 'error');
      }
    } else {
      resetSliderHandle();
    ...
    // Reset loader text and hide spinner / restore button
    const loaderText = document.getElementById('enhance-loading-text');
    if (loaderText) loaderText.textContent = 'Procesando imagen con Sharp...';
...
// ==========================================================================
// Client-side Super-Resolution (UpscalerJS / ESRGAN Slim 2x)
// ==========================================================================
//
// The server's /enhance endpoint no longer upscales. When `upscale` is requested,
// the frontend tries to run UpscalerJS (TensorFlow.js + WebGL) on the enhanced
// image. If anything fails (no WebGL, model load error, TF unavailable, etc.),
// we fall back to POST /api/images/:id/upscale which does a server-side Lanczos 2x.
//
let _upscalerInstance = null;
let _upscalerReady = false;
let _upscalerFailed = false;

/**
 * Lazily instantiates the UpscalerJS client with the ESRGAN Slim 2x model.
 * Returns true if the client is ready to upscale, false if it should fall back.
 */
async function getUpscalerClient() {
  if (_upscalerFailed) { console.info('[SR] Already failed init — using fallback.'); return null; }
  if (_upscalerReady) { console.info('[SR] Reusing ready instance.'); return _upscalerInstance; }

  // Feature-detect required globals
  if (typeof window.tf === 'undefined' || typeof window.Upscaler === 'undefined' || typeof window.ESRGANSlim2x === 'undefined') {
    console.warn('[SR] Globals not available:', {
      tf: typeof window.tf, Upscaler: typeof window.Upscaler, ESRGANSlim2x: typeof window.ESRGANSlim2x
    });
  ...
  try {
    console.info('[SR] Initializing TensorFlow.js...');
    await tf.setBackend('webgl').catch(() => {});
    await tf.ready();
    const backend = tf.getBackend();
    console.info(`[SR] TF backend: ${backend}`);
    if (backend !== 'webgl' && backend !== 'wasm') {
      console.warn(`[SR] Unsupported backend "${backend}". Falling back.`);
      _upscalerFailed = true;
      return null;
    }

    console.info('[SR] Loading ESRGAN Slim 2x model...');
    _upscalerInstance = new Upscaler({
      model: ESRGANSlim2x,
      warmupSizes: { patchSize: 64 }
    });
    _upscalerReady = true;
    console.info('[SR] UpscalerJS ready.');
    return _upscalerInstance;
  } catch (err) {
    console.error('[SR] Init failed:', err);
    _upscalerFailed = true;
    return null;
  }
}
...
/**
 * Runs the client-side ESRGAN 2x upscale on the image currently displayed as
 * the "enhanced" result. Returns a data URL (base64 PNG) or null on failure.
 *
 * @param {HTMLImageElement} imgEl  - The image element whose src will be upscaled.
 * @param {(pct:number)=>void} [onProgress] - Optional progress callback (0..1).
 * @returns {Promise<string|null>}  data: URL of the upscaled image, or null.
 */
async function runClientUpscale(imgEl, onProgress) {
  const upscaler = await getUpscalerClient();
  if (!upscaler) return null;

  try {
    console.info(`[SR] Executing ESRGAN 2x on ${imgEl.naturalWidth}x${imgEl.naturalHeight} image...`);
    const t0 = performance.now();
    const result = await upscaler.execute(imgEl, {
      output: 'base64',
      patchSize: 64,
      padding: 2,
      progress: (p) => { console.info(`[SR] Patch progress: ${(p * 100).toFixed(0)}%`); if (typeof onProgress === 'function') onProgress(p); }
    });
    const elapsed = ((performance.now() - t0) / 1000).toFixed(1);
    console.info(`[SR] ESRGAN execute completed in ${elapsed}s.`);
    return result; // already a data: URL string in browser mode
  } catch (err) {
    console.error('[SR] execute failed:', err);
    return null;
  }
}

/**
 * Full orchestration: given the just-enhanced image object (from /enhance), if
 * `upscale` was requested, attempt client-side ESRGAN 2x; on any failure, call
 * the server fallback `/api/images/:id/upscale`. Updates `state.currentImage`
 * and the UI in place.
 *
 * @param {Object} enhancedImage - The image object returned by /enhance.
 * @param {boolean} wantsUpscale - Whether the user checked "Súper-Resolución".
 * @returns {Promise<void>}
 */
async function applySuperResolution(enhancedImage, wantsUpscale) {
  if (!wantsUpscale) return;

  const loaderText = document.getElementById('enhance-loading-text');
  const t0 = performance.now();

  // Load the enhanced image into a detached <img> we can feed to UpscalerJS
  console.info('[SR] Preloading enhanced image for ESRGAN...', enhancedImage.enhanced_url);
  if (loaderText) loaderText.textContent = 'Preparando súper-resolución con IA...';
  const tmpImg = new Image();
  tmpImg.crossOrigin = 'anonymous';
  tmpImg.src = enhancedImage.enhanced_url;
  await new Promise((resolve, reject) => {
    tmpImg.onload = resolve;
    tmpImg.onerror = reject;
  }).catch(() => null);

  // If the image didn't load, we can't upscale client-side — go straight to fallback.
  if (!tmpImg.naturalWidth) {
    console.warn('[SR] Enhanced image failed to preload; falling back to server Lanczos.');
    showToast('La imagen no se pudo cargar para ESRGAN. Usando fallback Lanczos.', 'info');
    await serverUpscaleFallback(enhancedImage.id);
    return;
  }

  console.info(`[SR] Source: ${tmpImg.naturalWidth}x${tmpImg.naturalHeight}. Starting ESRGAN 2x...`);
  if (loaderText) loaderText.textContent = 'Mejorando resolución con IA (ESRGAN 2x)... 0%';

  // Show a tiny progress indicator on the spinner text while ESRGAN runs.
  const onProgress = (p) => {
    if (loaderText) loaderText.textContent = `Mejorando resolución con IA (ESRGAN 2x)... ${Math.round(p * 100)}%`;
  };

  const dataUrl = await runClientUpscale(tmpImg, onProgress);
  const elapsed = ((performance.now() - t0) / 1000).toFixed(1);

  if (dataUrl) {
    console.info(`[SR] ESRGAN 2x done in ${elapsed}s. Persisting to server...`);
    // Persist the AI-upscaled result to the server so it survives reloads.
    await persistClientUpscale(enhancedImage.id, dataUrl, tmpImg.naturalWidth, tmpImg.naturalHeight);
    return;
  }

  // Fallback: server-side Lanczos 2x
  console.warn(`[SR] ESRGAN failed after ${elapsed}s. Falling back to server Lanczos.`);
  showToast('ESRGAN no se pudo ejecutar. Aplicando fallback Lanczos en servidor.', 'info');
  await serverUpscaleFallback(enhancedImage.id);
}

/**
 * Calls POST /api/images/:id/upscale and patches the UI/state with the result.
 * This is the server-side Lanczos fallback.
 */
async function serverUpscaleFallback(imageId) {
  const loaderText = document.getElementById('enhance-loading-text');
  if (loaderText) loaderText.textContent = 'Mejorando resolución (Lanczos en servidor)...';

  try {
    // No maxWidth cap — always upscale by the requested factor.
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Error desconocido en el escalado.');
    ...
    fitComparisonCardToImage(state.currentImage);
    resetSliderHandle();
    const srcW = data.image.enhanced_width / 2;
    console.info(`[SR] Server Lanczos fallback: ${srcW}px -> ${data.image.enhanced_width}px`);
    showToast(`Súper-resolución aplicada (Lanczos en servidor): ${data.image.enhanced_width}x${data.image.enhanced_height}.`, 'success');
  } catch (err) {
    showToast(`No se pudo aplicar súper-resolución: ${err.message}`, 'error');
  }
}

/**
 * Uploads the AI-upscaled image (from ESRGAN client-side) to the server and
 * persists it as the new enhanced version. The blob is sent as multipart form
 * data. This ensures the result survives reloads and is downloadable.
 */
async function persistClientUpscale(imageId, dataUrl, srcW, srcH) {
  const loaderText = document.getElementById('enhance-loading-text');
  if (loaderText) loaderText.textContent = 'Guardando imagen mejorada con IA...';
  ...
  try {
    ...
    const formData = new FormData();
    formData.append('image', blob, 'esrgan_upscaled.png');
    formData.append('source_width', String(srcW));
    formData.append('source_height', String(srcH));

    const res = await fetch(`/api/images/${imageId}/upscale-client`, {
      method: 'POST',
      body: formData
    });
    const data = await res.json();
    ...
    fitComparisonCardToImage(state.currentImage);
    resetSliderHandle();
    console.info(`[SR] ESRGAN persisted: ${srcW}px -> ${data.image.enhanced_width}px (${formatBytes(data.image.enhanced_size)})`);
    showToast(`Súper-resolución aplicada con IA (ESRGAN 2x): ${data.image.enhanced_width}x${data.image.enhanced_height}.`, 'success');
  } catch (err) {
    console.error('[SR] Persist failed:', err);
    showToast(`No se pudo guardar la imagen escalada: ${err.message}`, 'error');
  }
}
